...
export const useArticlesStore = defineStore("articles", {
  ...
  actions: {
    ...
    async fetchArticles(filters: { author?: string; tag?: string; favorited?: string } = {}) {
      try {
        ...
        console.log("Requête envoyée à l'API :", `/articles?${params.toString()}`);
    ...
    async fetchComments(slug: string) {
      if (!slug) throw new Error("Slug manquant pour récupérer les commentaires.");
      try {
        console.log("Chargement des commentaires pour l'article avec le slug :", slug);
    ...
    async addComment(slug: string, comment: string) {
      if (!slug || !comment.trim()) throw new Error("Données manquantes pour ajouter un commentaire.");
      try {
    ...
    async deleteComment(slug: string, commentId: number) {
      if (!slug || !commentId) throw new Error("Données manquantes pour supprimer le commentaire.");
      try {
        console.log("Suppression du commentaire ID :", commentId, "pour l'article avec le slug :", slug);
        await axios.delete(`/articles/${slug}/comments/${commentId}`);
        this.comments = this.comments.filter((comment) => comment.id !== commentId); // Supprime localement
    ...
    async favoriteArticle(slug: string) {
      if (!slug) throw new Error("Slug manquant pour ajouter l'article aux favoris.");
      try {
    ...
    async unfavoriteArticle(slug: string) {
      if (!slug) throw new Error("Slug manquant pour retirer l'article des favoris.");
      try {
    ...
    // Modifier un article
    async updateArticle(slug: string, data: { title: string; description: string; body: string }) {
      if (!slug || !data.title || !data.description || !data.body)
        throw new Error("Données manquantes pour modifier l'article.");
      try {
        console.log("Modification de l'article avec le slug :", slug, "Données :", data);
    ...
    async deleteArticle(slug: string) {
      ...
      try {
        ...
        console.error("Erreur lors de la suppression de l'article ou des commentaires :", error);
